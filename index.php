...
<span itemprop="postalCode" content="'.display_if_exists_simple($Postal).display_if_exists_simple($Zip).'">'.display_if_exists_simple($Postal).display_if_exists_simple($Zip).'</span>, <span itemprop="addressCountry" content="USA">USA</span></p><small>mailing address</small></blockquote></div>';
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <?php echo isset($vid) ? '<p><a class="btn btn-default" href="mailto:<?php echo $email;?>?subject=edit entry from '.$domain.'&body=Hi,please edit my company on your database : '.$_SERVER['REQUEST_URI'].' Here are the changes I need :">Edit Entry </a></p>' : ' '; ?>
      </div>
      <div class="col-xs-6 col-lg-4">
     ...
     <?php echo isset($vid) ? '<p><a class="btn btn-default" HREF="mailto:'.$email.'?subject=remove entry from '.$domain.'&body=Hi,please remove my company from your database : '.$_SERVER['REQUEST_URI'].'">Remove Entry</a></p>' : ' '; ?>
      </div>
    </div>
  </div>
</section>
