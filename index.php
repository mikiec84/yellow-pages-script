...
<section>
  ...
  <?php  display_if_exists($Business_Name, 'company name') ?>
    ...
    ?>
      ...
      <?php
    if (!isset($Address)) { display_if_exists($average_earning, 'average earning') };
    if (!isset($Address)) { display_if_exists($primary_contact_name, 'primary contact name')};
    if (!isset($Address)) { display_if_exists($primary_contact_phone, 'primary contact phone')};
    if (!isset($Address)) { display_if_exists($primary_contact_email, 'primary contact email')};
    if (!isset($Address)) { display_if_exists($other_contacts, 'other contacts')};
    if (!isset($Address)) { display_if_exists($year_founded, 'year founded')};
    if (!isset($Address)) { display_if_exists($industry, 'industry')};
    if (!isset($Address)) { display_if_exists($naics_primary, 'naics primary')};
    if (!isset($Address)) { display_if_exists($naics_secondary, 'naics secondary')};
    if (!isset($Address)) { display_if_exists($sic_primary, 'sic primary')};
    if (!isset($Address)) { display_if_exists($sic_secondary, 'sic secondary')};
    if (!isset($Address)) { display_if_exists($sales_volume, 'sales volume')};
    if (!isset($Address)) { display_if_exists($number_of_employees, 'number of employees')};
    if (!isset($Address)) { display_if_exists($products_services, 'products services')};
    if (!isset($Address)) { display_if_exists($business_activity, 'business activity')};
    if (!isset($Address)) { display_if_exists($profile, 'profile')};
    if (!isset($Address)) { display_if_exists($Category, 'Category 1')};
    if (!isset($Address)) { display_if_exists($Category_2, 'Category 2')};
    if (!isset($Address)) { display_if_exists($Category_3, 'Category 3')};
    ?>
